 ...
 */
class SuperTable extends ComponentEx_1.PureComponentEx {
    constructor(props) {
        super(props);
        this.mRowRefs = {};
        ...
        this.renderBody = (visibleAttributes) => {
            ...
            return (React.createElement(MyTable_1.TBody, { domRef: this.setBodyRef }, sortedRows.map((row, idx) => this.renderRow(row, idx < 40, visibleAttributes))));
        };
        ...
        this.setRowHighlight = (rowId, highlighted) => {
            this.setState(storeHelper_1.setSafe(this.state, ['rowState', rowId, 'highlighted'], highlighted));
        };
            ...
            React.createElement("div", { className: 'table-container-inner' },
                React.createElement("div", { className: 'table-main-pane', ref: this.mainPaneRef, tabIndex: 0, onKeyDown: this.handleKeyDown },
                    React.createElement(MyTable_1.Table, { hover: true },
                        ...
                        showHeader === false ? null : React.createElement(MyTable_1.THead, { className: 'table-header', domRef: this.setHeadRef, style: { transform: 'translate(0, 0)' } },
    ...
    renderFooter() {
        ...
        const { rowState } = this.state;
        const multiActions = actions.filter((action) => action.multiRowAction === undefined || action.multiRowAction);
        ...
        return (React.createElement("div", { className: 'table-footer-placeholder' },
            React.createElement("div", { className: 'table-footer' },
                React.createElement(IconBar_1.default, { className: 'menubar' }, multiActions.map((action, idx) => React.createElement(ToolbarIcon_1.default, { key: idx, icon: action.icon, text: action.title, instanceId: selected, onClick: action.action }))),
    ...
    renderRow(data, initVisible, visibleAttributes) {
        ...
        const rowActions = actions.filter((action) => (action.singleRowAction === undefined) || action.singleRowAction);
        return (React.createElement(TableRow_1.default, { t: t, tableId: tableId, id: data.id, key: data.id, data: calculatedValues[data.id], rawData: data.data, attributes: visibleAttributes, sortAttribute: sortAttribute !== undefined ? sortAttribute.id : undefined, actions: rowActions, language: language, onClick: this.selectRow, selected: storeHelper_1.getSafe(rowState, [data.id, 'selected'], false), highlighted: storeHelper_1.getSafe(rowState, [data.id, 'highlighted'], false), domRef: this.setRowRef, container: this.mScrollRef, initVisible: initVisible, onHighlight: this.setRowHighlight }));
    }
...
function registerTableAttribute(instanceProps, group, attribute) {
...
exports.default = ComponentEx_1.translate(['common'], { wait: false })(ComponentEx_1.connect(mapStateToProps, mapDispatchToProps)(ComponentEx_1.extend(registerTableAttribute)(SuperTable)));
