...
class SuperTable extends ComponentEx_1.ComponentEx {
    constructor(props) {
        ...
        this.renderBody = (visibleAttributes) => {
            ...
            return (React.createElement(MyTable_1.TBody, null, sortedRows.map((row, idx) => this.renderRow(row, visibleAttributes))));
        };
        ...
        this.renderDetails = () => {
            ...
            return (React.createElement(TableDetail_1.default, { t: t, rowIds: rowIdsDelayed, rowData: calculatedValues, rawData: data, attributes: detailAttributes, language: language, show: detailsOpen, title: detailsTitle, onToggleShow: this.toggleDetails }));
        };
        this.renderHeaderField = (attribute) => {
            ...
            if (attributeState.enabled) {
                ...
                return (React.createElement(HeaderCell_1.default, { className: classes.join(' '), key: attribute.id, attribute: attribute, state: attributeState, doFilter: true, onSetSortDirection: this.setSortDirection, onSetFilter: this.setFilter, ref: this.setHeaderCellRef, t: t }, attribute.filter !== undefined ? (React.createElement(attribute.filter.component, { filter: filt, attributeId: attribute.id, t: t, onSetFilter: this.setFilter })) : null));
            }
            else {
                return null;
            }
    ...
    renderFooter() {
        ...
        return (React.createElement("div", { className: 'table-footer-placeholder' },
            React.createElement("div", { className: 'table-footer' },
                React.createElement(IconBar_1.default, { className: 'menubar', t: t }, multiRowActions.map((action, idx) => React.createElement(ToolbarIcon_1.default, { key: idx, icon: action.icon, text: action.title, instanceId: selected, onClick: action.action }))),
    ...
    renderRow(rowId, visibleAttributes) {
        ...
        return (React.createElement(TableRow_1.default, { t: t, tableId: tableId, id: rowId, key: rowId, data: calculatedValues[rowId], rawData: data[rowId], attributes: visibleAttributes, sortAttribute: sortAttribute !== undefined ? sortAttribute.id : undefined, actions: singleRowActions, language: language, onClick: this.selectRow, selected: storeHelper_1.getSafe(rowState, [rowId, 'selected'], false), highlighted: storeHelper_1.getSafe(rowState, [rowId, 'highlighted'], false), domRef: this.setRowRef, container: this.mScrollRef, visible: this.state.rowVisibility[rowId] === true, onSetVisible: this.setRowVisible, onHighlight: this.setRowHighlight }));
    }
