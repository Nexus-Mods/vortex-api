...
const getButtonClasses = ({ buttonType, filled, isDisabled, isLoading, isResponsive, size, }) => {
    ...
    switch (buttonType) {
        case "primary":
            classes.push("tw:bg-primary-moderate tw:text-neutral-inverted", canHover ? "tw:hover:bg-primary-subdued" : "");
            break;
        case "secondary":
        ...
        case "tertiary":
            ...
            break;
        case "success":
            classes.push("tw:bg-success-moderate tw:text-neutral-inverted", canHover ? "tw:hover:bg-success-subdued" : "");
            break;
        case "premium":
            classes.push("tw:bg-premium-moderate tw:text-neutral-strong", canHover ? "tw:hover:bg-premium-subdued" : "");
            break;
    }
    return classes;
};
const ButtonIcon = ({ className, icon, isLoading = false, path, wrapperClassName, }) => {
    ...
    if (!!icon) {
        return (React.createElement("span", { className: (0, utils_1.joinClasses)([
