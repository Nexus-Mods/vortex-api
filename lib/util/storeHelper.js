...
function setSafe(state, path, value) {
    ...
    const firstElement = path[0];
    const copy = Array.isArray(state) ? state.slice() : Object.assign({}, state); // "as any" is a workaround for
...
function setDefaultArray(state, path, fallback) {
    const firstElement = path[0];
    const copy = Array.isArray(state) ? state.slice() : Object.assign({}, state);
    if (path.length === 0) {
...
function currentGame(store) {
    ...
    if (knownGames !== null && knownGames !== undefined) {
        ...
        return waitUntil(() => {
            knownGames = getSafe(store.getState(), ["session", "gameMode", "known"], null);
