...
class Steam {
    ...
    launchGame(appInfo, api) {
        ...
        if (this.isCustomExecObject(appInfo) &&
            ...
            return this.getPosixPath(appInfo).then((posix) => (0, opn_1.default)(posix).catch((err) => bluebird_1.default.resolve()));
        }
        const info = !!appInfo.steamAppId ? appInfo.steamAppId.toString() : appInfo;
        return this.getExecInfo(info).then((execInfo) => api.runExecutable(execInfo.execPath, execInfo.arguments, {
    ...
    identifyGame(gamePath, fallback) {
        ...
        return bluebird_1.default.resolve(fallback(gamePath)).then((fbResult) => {
            if (fbResult !== custom) {
    ...
    parseManifests() {
        return this.resolveSteamPaths().then((steamPaths) => bluebird_1.default.mapSeries(steamPaths, (steamPath) => {
            ...
            return bluebird_1.default.resolve(fsOG.readdir(steamAppsPath))
                .then((names) => {
                const filtered = names.filter((name) => name.startsWith("appmanifest_") &&
                    path.extname(name) === ".acf");
                (0, log_1.log)("debug", "got steam manifests", { manifests: filtered });
                return bluebird_1.default.map(filtered, (name) => fs
                ...
                return appsData
                    ...
                    try {
                        ...
                        (0, log_1.log)("warn", "failed to parse steam manifest", {
                            name,
                            error: err.message,
                        });
                        return undefined;
                ...
                .catch((err) => {
                (0, log_1.log)("warn", "Failed to read steam library", {
                    path: steamPath,
                    error: err.message,
                });
            });
