    ...
    switch (typeof input) {
        case "object": {
            ...
            if (attributes.length === 0) {
                attributes = (0, util_1.getAllPropertyNames)(flatErr || {}).filter((key) => ["message", "error", "stack", "context"].indexOf(key) === -1);
            }
            const details = attributes.length === 0
                ? undefined
        ...
        default: {
            return {
                message: "Unknown exception: " + (0, util_2.inspect)(input),
                title,
                subtitle,
            };
        }
