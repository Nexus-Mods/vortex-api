...
class ReduxProp {
    ...
    calculate() {
        ...
        const values = this.mInputs.map(valPath => storeHelper_1.getSafe(this.mApi.store.getState(), valPath, undefined));
    ...
    subscribe() {
        ...
        this.mUnsubscribe = this.mApi.store.subscribe(() => {
            const changed = this.mInputs.find(valPath => storeHelper_1.getSafe(oldState, valPath, undefined)
                !== storeHelper_1.getSafe(this.mApi.store.getState(), valPath, undefined));
