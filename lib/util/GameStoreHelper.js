...
class GameStoreHelper {
                ...
                for (const storeQuery of query[storeId]) {
                    ...
                    try {
                        ...
                        if (!(err instanceof IGameStore_1.GameEntryNotFound)) {
                            (0, log_1.log)("error", "Failed to look up game", {
                                storeId,
                                appid: storeQuery.id,
                    ...
                    if (result !== undefined) {
                        result.priority =
                            (_c = (_a = storeQuery.prefer) !== null && _a !== void 0 ? _a : (_b = this.mStoresDict[result.gameStoreId]) === null || _b === void 0 ? void 0 : _b.priority) !== null && _c !== void 0 ? _c : 100;
    ...
    launchGameStore(api, gameStoreId, parameters, askConsent = false) {
        ...
        const launchStore = () => this.isGameStoreInstalled(gameStoreId).then((gamestoreInstalled) => {
    ...
    reloadGames(api) {
        ...
        (0, log_1.log)("info", "reloading game store games", stores.map((store) => store.id).join(", "));
        return bluebird_1.default.each(stores, (store) => (store === null || store === void 0 ? void 0 : store.reloadGames) !== undefined
            ...
            : bluebird_1.default.resolve()).then(() => {
            var _a;
     ...
     */
    storeIds() {
        return this.mStores.sort((lhs, rhs) => lhs.priority - rhs.priority);
    }
    ...
    getStores() {
        ...
        //  to load the stores.
        try {
            this.mStores = (0, getGame_1.getGameStores)().filter((store) => !!store);
    ...
    findGameEntry(searchType, pattern, storeId) {
        ...
        if (!!storeId) {
            try {
                ...
                (0, log_1.log)("warn", "Game entry not found in specified store", {
                    pattern: name,
                    storeId,
        ...
        const gameStores = (!!queriedStore ? [queriedStore] : this.getStores()).filter((store) => !!store);
        if (gameStores === undefined || gameStores.length === 0) {
            const stores = gameStores !== undefined
                ? gameStores.map((store) => store.id).join(", ")
                : "";
            (0, log_1.log)("debug", "Game entry not found", {
                pattern: name,
                availableStores: stores,
            });
            return bluebird_1.default.reject(new IGameStore_1.GameEntryNotFound(name, stores));
        }
        return bluebird_1.default.reduce(gameStores, (accum, store) => store
            .allGames()
            .then((entries) => {
            ...
            .catch(IGameStore_1.GameEntryNotFound, () => bluebird_1.default.resolve(accum)), []).then((foundEntries) => {
