...
function shouldAllowReport(err, options) {
    if ((options === null || options === void 0 ? void 0 : options.allowReport) === false ||
        ...
        err instanceof CustomErrors_1.UserCanceled) {
        return false;
    }
    if (err instanceof CustomErrors_1.UserCanceled) {
        (0, log_1.log)("debug", "Incorrectly handled UserCanceled error", {
            stack: err.stack,
        });
    }
    ...
    return new bluebird_1.default((resolve, reject) => {
        ...
        (0, tmp_1.file)({
            ...
            fs.writeAsync(fd, data, 0, data.byteLength, 0)
                ...
                .catch((innerErr) => {
                (0, log_1.log)("error", "failed to write attachment data to file", {
...
function zipFiles(files) {
    ...
    return new bluebird_1.default((resolve, reject) => {
        (0, tmp_1.tmpName)({
            postfix: ".7z",
        }, (err, tmpPath) => (err !== null ? reject(err) : resolve(tmpPath)));
...
function showError(dispatch, title, details, options) {
    ...
    if (((_c = (_b = options.extension) === null || _b === void 0 ? void 0 : _b.info) === null || _c === void 0 ? void 0 : _c.issueTrackerURL) !== undefined) {
        ...
        extIssueTrackerURL =
            "https://github.com/" + ((_g = options.extensionRemote) === null || _g === void 0 ? void 0 : _g.github) + "/issues";
    }
            ...
            actions.push({
                ...
                action: () => {
                    dispatch((0, notifications_1.showDialog)("info", "Reporting to extension author", {
                        ...
                        {
                            label: "Continue",
                            action: () => (0, opn_1.default)(extIssueTrackerURL).catch(() => null),
                        },
                    ]));
                },
            });
...
function prettifyNodeErrorMessage(err, options, fileName) {
    ...
    if (err instanceof CustomErrors_1.ThirdPartyError || err instanceof CustomErrors_1.ArchiveBrokenError) {
        return {
            message: err.message,
            allowReport: false,
        };
...
function renderCustomError(err) {
    ...
    if (attributes.length === 0) {
        attributes = Object.keys(err || {}).filter((key) => !isPrivateField(key) && !HIDE_ATTRIBUTES.has(key));
    }
    if (attributes.length > 0) {
...
function renderError(err, options) {
    ...
    if (typeof err === "string") {
        ...
        if (attributes.length === 0) {
            attributes = Object.keys(flatErr || {}).filter((key) => !isPrivateField(key) && !HIDE_ATTRIBUTES.has(key));
        }
        if (attributes.length > 0) {
