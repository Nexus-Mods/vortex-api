 ...
 */
function showError(dispatch, message, details, isHTML = false, id, allowReport = true) {
    const err = renderError(details);
    log_1.log('error', message, err.message);
    ...
    if (allowReport) {
        actions.push({
            ...
            action: () => {
                ...
                return errorHandling_1.createErrorReport('Error', {
                    message,
                    stack,
                    details: repDetails,
                }, ['bug']);
