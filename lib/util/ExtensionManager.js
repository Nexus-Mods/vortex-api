...
class ExtensionManager {
        ...
        this.runExecutable = (executable, args, options) => {
            ...
            return (options.suggestDeploy === true ? this.checkDeploy() : Promise.resolve(true))
                ...
                try {
                    ...
                    if (err.errno === 'EACCES') {
                        return resolve(vortex_run_1.runElevated(shortid_1.generate(), runElevatedCustomTool_1.default, {
                            toolPath: executable,
                            toolCWD: cwd,
                            parameters: args,
                            environment: env,
    ...
    setStore(store) {
        ...
        if (electron_1.ipcRenderer !== undefined) {
            ...
            electron_1.ipcRenderer.on('show-error-notification', (event, message, details, options) => this.mApi.showErrorNotification(message, details, options));
