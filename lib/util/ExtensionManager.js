...
const notifications_2 = require("../actions/notifications");
...
const runElevatedCustomTool_1 = require("./runElevatedCustomTool");
...
const fs = require("fs");
const modmeta = lazyRequire_1.default(() => require('modmeta-db'));
    ...
    unloadIncompatible(furtherAPIs, allExtensions) {
        ...
        this.getCalls('requireVersion').forEach(call => {
            if ((process.env.NODE_ENV !== 'development')
                ...
                util_1.setdefault(incompatibleExtensions, call.extension, []).push({ id: 'unsupported-version' });
            }
        });
...
class ExtensionManager {
    constructor(initStore, eventEmitter) {
        ...
        this.registerProtocol = (protocol, def, callback) => {
            ...
            if (def) {
                if (process.execPath.endsWith('electron.exe')) {
                    ...
                    app.setAsDefaultProtocolClient(protocol, process.execPath, [getVortexPath_1.default('package'), '-d']);
                }
                else {
        ...
        this.queryDeploy = () => {
            const autoDeploy = storeHelper_1.getSafe(this.mApi.store.getState(), ['settings', 'automation', 'deploy'], true);
            if (autoDeploy) {
                ...
                return this.mApi.showDialog('question', 'Deploy now?', {
                    ...
                    switch (result.action) {
                        case 'Skip': return Promise.resolve('skip');
                        case 'Deploy': return Promise.resolve('yes');
                        default: return Promise.resolve('cancel');
                    }
                });
            }
        };
        this.runExecutable = (executable, args, options) => {
            ...
            return (options.suggestDeploy === true ? this.checkDeploy() : Promise.resolve(true))
                .then(cont => cont ? new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    const child = child_process_1.spawn(runExe, args.map(arg => arg.replace(/"/g, '')), spawnOptions);
                    child
                        .on('error', err => {
                        reject(err);
                    })
                        ...
                        if (code !== 0) {
                            ...
                            log_1.log('warn', 'child process exited with code: ' + code, {});
                        }
                        resolve();
                    });
                    child.stderr.on('data', chunk => {
                        log_1.log('error', executable + ': ', chunk.toString());
                    });
                }
                catch (err) {
                    const ipcPath = shortid_1.generate();
                    this.startIPC(ipcPath);
                    if (err.errno === 'EACCES') {
                        return resolve(vortex_run_1.runElevated(ipcPath, runElevatedCustomTool_1.default, {
                            toolPath: executable,
                            toolCWD: cwd,
                            parameters: args,
                            environment: env,
                ...
                .catch(vortex_run_1.Win32Error, err => err.code === 5
    ...
    static getExtensionPaths() {
        ...
        return [
            ...
            getVortexPath_1.default('bundledPlugins'),
        ];
    }
    ...
    setStore(store) {
        ...
        this.mApi.showErrorNotification =
            (message, details, options) => {
                message_1.showError(store.dispatch, message, details, options);
            };
        this.mApi.showDialog =
            (type, title, content, actions) => {
                return store.dispatch(notifications_2.showDialog(type, title, content, actions));
            };
        ...
        if (electron_1.ipcRenderer !== undefined) {
            ...
            electron_1.ipcRenderer.on('show-error-notification', (event, message, details, options) => this.mApi.showErrorNotification(message, details, options || undefined));
    ...
    selectFile(options) {
        return new Promise((resolve, reject) => {
            ...
            dialog.showOpenDialog(win, fullOptions, (fileNames) => {
    ...
    deregisterProtocol(protocol) {
        ...
        if (process.execPath.endsWith('electron.exe')) {
            // make it work when using the development version
            app.removeAsDefaultProtocolClient(protocol, process.execPath, [getVortexPath_1.default('package'), '-d']);
        }
        else {
    ...
    checkDeploy() {
        return this.queryDeploy()
            ...
            if (shouldDeploy === 'yes') {
                ...
                return new Promise((resolve, reject) => {
                    this.mApi.events.emit('await-activation', (err) => {
                        if (err !== null) {
                            reject(err);
                        }
                        else {
    ...
    startIPC(ipcPath) {
        const ipcServer = new nodeIPC.IPC();
        ipcServer.serve(ipcPath, () => null);
        ipcServer.server.start();
        ipcServer.server.on('connect', () => {
            log_1.log('debug', 'ipc client connected');
        });
        ipcServer.server.on('socket.disconnected', () => {
            log_1.log('debug', 'socket disconnect');
            ipcServer.server.stop();
        });
        ipcServer.server.on('log', (data) => {
            log_1.log(data.level, data.message, data.meta);
        });
        ipcServer.server.on('finished', () => null);
        ipcServer.server.on('error', nodeIPCErr => {
            log_1.log('error', 'ipcServer err', nodeIPCErr);
        });
    }
    loadDynamicExtension(extensionPath) {
        ...
        if (fs.existsSync(indexPath)) {
            return {
                ...
                initFunc: vortex_run_1.dynreq(indexPath).default,
                path: extensionPath,
            };
        }
        else {
