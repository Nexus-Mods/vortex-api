...
class ExtensionManager {
    ...
    constructor(initStore, eventEmitter) {
        ...
        this.genMd5Hash = (filePath, progressFunc) => {
            ...
            return (0, util_1.toPromise)(cb => (0, vortexmt_1.fileMD5)(filePath, cb, progressHash))
                .then((result) => (lastProgress === 0)
                ? fsVortex.statAsync(filePath).then(stats => stats.size).catch(() => 0)
                    .then(numBytes => bluebird_1.default.resolve({
                    md5sum: result,
                    numBytes
                }))
                : bluebird_1.default.resolve({
                    ...
                    child
                        ...
                        if ((game === 'fallout3') && (code === 0xC0000135)) {
                            ...
                            if (options.expectSuccess) {
                                ...
                                const err = new Error(`Failed to run "${sanitizedExecutable}": "${sanitizedLastLine} (${exitCodeHex})"`);
                                err.exitCode = code;
                                return reject(err);
                            }
                        }
