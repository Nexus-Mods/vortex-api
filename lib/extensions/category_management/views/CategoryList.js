...
const tree = lazyRequire_1.default('react-sortable-tree');
const nop = () => undefined;
/**
...
class CategoryList extends ComponentEx_1.ComponentEx {
    constructor(props) {
        ...
        this.generateNodeProps = (rowInfo) => {
            ...
            return {
                buttons: [
                    (React.createElement(IconBar_1.default, { group: 'category-icons', staticElements: actions, instanceId: rowInfo.node.categoryId, dropdown: true })),
                ],
            };
        };
    ...
    render() {
        ...
        if ((expandedTreeData !== undefined) && (expandedTreeData.length > 0)) {
            return (React.createElement("div", { className: 'categories-dialog' },
                ...
                React.createElement(Tree, { treeData: expandedTreeData, onChange: nop, onVisibilityToggle: this.toggleVisibility, onMoveNode: this.moveNode, style: { height: '95%' }, searchMethod: this.searchMethod, searchQuery: searchString, searchFocusOffset: searchFocusIndex, searchFinishCallback: this.searchFinishCallback, getNodeKey: this.getNodeKey, generateNodeProps: this.generateNodeProps })));
        }
        else {
            return (React.createElement("div", { style: { height: '90%' } },
                React.createElement(TooltipControls_1.Button, { id: 'add-category', tooltip: t('Add Root Category'), onClick: this.addRootCategory },
                    React.createElement(Icon_1.default, { name: 'folder-add' })),
                React.createElement(IconBar_1.default, { group: 'categories-icons', staticElements: null })));
        }
    }
    ...
    applyExpand(treeData, showEmpty, expanded) {
        let filtered;
        if (showEmpty) {
            const { categories } = this.props;
    ...
    refreshTree(props) {
        ...
        if (categories !== undefined) {
            if (Object.keys(categories).length !== 0) {
                ...
                const globalPersistentPath = path.join(electron_1.remote.app.getPath('userData'), 'state');
                onShowError('An error occurred loading the categories.', 'Can\'t read local categories. If you manually edited global_persistent you probably ' +
