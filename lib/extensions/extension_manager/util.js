...
function fetchAvailableExtensions(forceCache, forceDownload = false) {
    if (caches.__availableExtensions === undefined ||
        forceCache ||
        forceDownload) {
...
function downloadAndInstallExtension(api, ext) {
    ...
    const sourceName = (0, util_1.truthy)(ext.modId) ? "nexusmods.com" : "github.com";
    return dlPromise
        .then((dlIds) => {
        ...
        const extDetail = availableExtensions.extensions.find((iter) => (ext.modId === undefined || iter.modId === ext.modId) &&
            (ext.fileId === undefined || iter.fileId === ext.fileId) &&
            ext.name === iter.name);
        const info = extDetail !== undefined
            ? Object.assign(Object.assign({}, _.pick(extDetail, [
                "id",
                "name",
                "author",
                "version",
                "type",
            ])), { bundled: false, description: extDetail.description.short, modId: ext.modId }) : undefined;
        const state = api.store.getState();
...
function downloadFile(url, outputPath) {
    return bluebird_1.default.resolve((0, network_1.rawRequest)(url)).then((data) => fs.writeFileAsync(outputPath, data));
}
function downloadGithubRawRecursive(repo, source, destination) {
    ...
    return bluebird_1.default.resolve((0, network_1.rawRequest)(apiUrl, { encoding: "utf8" })).then((content) => {
        ...
        const repoFiles = data
            .filter((iter) => iter.type === "file")
            .map((iter) => iter.name);
        const repoDirs = data
            .filter((iter) => iter.type === "dir")
            .map((iter) => iter.name);
        return bluebird_1.default.map(repoFiles, (fileName) => downloadFile(githubRawUrl(repo, GAMES_BRANCH, `${source}/${fileName}`), path.join(destination, fileName))).then(() => bluebird_1.default.map(repoDirs, (fileName) => {
...
function downloadGithubRaw(api, ext) {
    ...
    const existing = Object.keys(files).find((dlId) => {
        var _a;
        return ((_a = files[dlId].game) !== null && _a !== void 0 ? _a : []).includes(constants_1.SITE_ID) &&
