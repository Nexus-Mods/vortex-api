...
function findModByRef(reference, mods, source) {
    if (!reference) {
        (0, log_1.log)("error", "findModByRef called with undefined reference", {
            source,
    ...
    if (reference["md5Hint"] !== undefined &&
        ...
        const result = Object.keys(mods).find((dlId) => { var _a; return ((_a = mods[dlId].attributes) === null || _a === void 0 ? void 0 : _a.fileMD5) === reference["md5Hint"]; });
        if (result !== undefined) {
            return mods[result];
        }
    }
...
function lookupFulfills(lookup, reference) {
    ...
    const refRepo = reference.repo;
    if ((refRepo === null || refRepo === void 0 ? void 0 : refRepo.fileId) !== undefined && !(0, testModReference_1.isFuzzyVersion)(versionMatch)) {
        const lookupFileId = (_b = (_a = value === null || value === void 0 ? void 0 : value.details) === null || _a === void 0 ? void 0 : _a.fileId) === null || _b === void 0 ? void 0 : _b.toString();
    ...
    return ((gameId === undefined || gameId === value.gameId) &&
        ...
        (fileExpression === undefined ||
            (value.fileName !== undefined &&
                (0, minimatch_1.default)(value.fileName, fileExpression))) &&
    ...
    const temp = input
        ...
        if (lhs.collateral.length !== rhs.collateral.length) {
            ...
            try {
                ...
                (0, log_1.log)("error", "failed to compare version", {
                    lhs: fileVerL,
                    rhs: fileVerR,
                });
...
function findDownloadByRef(reference, downloads) {
    if (reference["md5Hint"] !== undefined) {
        const result = Object.keys(downloads).find((dlId) => downloads[dlId].fileMD5 === reference["md5Hint"]);
        if (result !== undefined) {
            return result;
        }
    }
    ...
    try {
        ...
        const existing = Object.keys(downloads)
            .filter((dlId) => {
            var _a, _b, _c, _d;
            const download = downloads[dlId];
            const isRelevantDownload = download.game.includes(reference.gameId) &&
                !((_a = download.modInfo) === null || _a === void 0 ? void 0 : _a.collectionSlug);
            ...
            return fuzzy || bundled
                ...
                : (0, testModReference_1.default)(lookup, reference, undefined, fuzzy) ||
                    (0, testModReference_1.testRefByIdentifiers)(identifiers, reference);
        })
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (!downloadId) {
                ...
                if (urlFromHint) {
                    (0, log_1.log)("info", "url from dependency", {
                        urlFromHint,
                        md5: rule.reference.fileMD5,
                    });
                }
            }
            lookupResults = yield api.lookupModReference(rule.reference, {
                requireURL: true,
            });
            const subRules = [
            ...
            if (urlFromHint) {
                ...
                if (((_p = rule.downloadHint) === null || _p === void 0 ? void 0 : _p.mode) === "browse") {
                    node.reresolveDownloadHint = () => lookupDownloadHint(api, rule.downloadHint).then((dlHintRes) => {
