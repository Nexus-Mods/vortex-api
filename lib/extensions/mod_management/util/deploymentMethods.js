...
function getSupportedActivators(state) {
    ...
    const modTypes = Object.keys(modPaths).filter((typeId) => (0, util_1.truthy)(modPaths[typeId]));
...
function getCurrentActivator(state, gameId, allowDefault) {
    ...
    const types = Object.keys(modPaths).filter((typeId) => (0, util_1.truthy)(modPaths[typeId]));
