...
function getActiveCollectionSession(state, sourceModId) {
    const collectionsState = (0, storeHelper_1.getSafe)(state, ['session', 'collections'], undefined);
    const activeSession = collectionsState === null || collectionsState === void 0 ? void 0 : collectionsState.activeSession;
...
class InstallManager {
    ...
    handleDownloadFinished(api, downloadId) {
        ...
        const dependency = {
            ...
            patches: (_b = matchingRule.extra) === null || _b === void 0 ? void 0 : _b.patches,
            installerChoices: (_c = matchingRule.extra) === null || _c === void 0 ? void 0 : _c.installerChoices,
     ...
     *                                 the auto-detection.
     */
    install(archiveId, archivePath, downloadGameIds, api, info, processDependencies, enable, callback, forceGameId, fileList, unattended, forceInstaller, allowAutoDeploy, sourceModId) {
    ...
    queueInstallation(api, dep, downloadId, gameId, sourceModId, recommended, phase = 0) {
        ...
        const alreadyActive = Array.from(this.mActiveInstalls.values()).some(inst => inst.installId.startsWith(depKey));
        const alreadyPending = Array.from(this.mPendingInstalls.keys()).some(key => key.startsWith(depKey));
    ...
    pollPhaseSettlement(api, sourceModId, options) {
        ...
        return new bluebird_1.default((resolve) => {
            const poll = () => {
                ...
                //   resolveOnSettle: options.resolveOnSettle
                // });
                ...
                if (phaseLogicallyComplete && installationsComplete) {
                    ...
                    else if (options.resolveOnSettle) {
                        // Always try to advance phases when settled, even without deployment
                        if (phaseState) {
                            this.maybeAdvancePhase(sourceModId, api);
                        }
                        resolve();
    ...
    checkCollectionPhaseStatus(api, sourceModId, phase) {
        ...
        const { isComplete: phaseComplete } = checkPhaseCompletion(currentPhaseMods);
        ...
        allDownloadedMods.forEach((mod) => {
            ...
            const downloadId = getReadyDownloadId(downloads, (_a = mod.rule) === null || _a === void 0 ? void 0 : _a.reference, (id) => this.hasActiveOrPendingInstallation(id));
            if (downloadId) {
                modsNeedingRequeue++;
            }
        });
    ...
    hasActiveOrPendingInstallation(archiveId) {
        const hasPending = this.mPendingInstalls.has(archiveId);
        let hasActive = false;
        for (const activeInstall of this.mActiveInstalls.values()) {
    ...
    reQueueDownloadedMods(api, sourceModId, allMods, currentPhase) {
        ...
        if (!phaseState)
            return;
        const downloads = api.getState().persistent.downloads.files;
        // Only requeue mods from the specified phase (or earlier if needed)
        const allDownloadedMods = allMods.filter((mod) => { var _a; return mod.status === 'downloaded' && ((_a = mod.phase) !== null && _a !== void 0 ? _a : 0) === currentPhase; });
        ...
        allDownloadedMods.forEach((mod) => {
            ...
            // Only requeue mods that are in the current allowed phase or earlier
            // This prevents infinite loops where future phase mods get requeued but then blocked by phase gating
            if (modPhase !== phaseState.allowedPhase) {
                (0, log_1.log)('debug', 'Skipping mod - phase not allowed yet', { modPhase, allowedPhase: phaseState.allowedPhase });
                return; // Skip this mod, it will be processed when its phase is allowed
            }
            let downloadId = (0, dependencies_1.findDownloadByRef)((_b = mod.rule) === null || _b === void 0 ? void 0 : _b.reference, downloads);
            if (!downloadId) {
                // If download can't be found and mod is optional (recommends), don't block progression
            ...
            if (downloads[downloadId].state === 'finished') {
                const hasPendingOrActive = this.hasActiveOrPendingInstallation(downloadId);
                (0, log_1.log)('debug', 'Requeue check', { downloadId, hasPendingOrActive });
    ...
    scheduleDeployOnPhaseSettled(api, sourceModId, phase) {
        ...
        const deploymentPromise = new Promise((resolve) => {
            this.pollPhaseSettlement(api, sourceModId, {
                ...
                .then(() => resolve())
                .catch(err => {
                (0, log_1.log)('error', 'Error in phase deployment polling', { sourceModId, phase, error: err.message });
                resolve();
            });
        });
                ...
                if (collectionMod === null || collectionMod === void 0 ? void 0 : collectionMod.rules) {
                    collectionMod.rules.forEach((rule) => {
                        ...
                        if (rulePhase === curr && ((_c = rule.reference) === null || _c === void 0 ? void 0 : _c.tag)) {
                            const downloadId = getReadyDownloadId(downloads, rule.reference, (id) => this.hasActiveOrPendingInstallation(id));
                            if (downloadId) {
                                this.handleDownloadFinished(api, downloadId);
                            }
                        }
    ...
    doInstallDependencies(api, gameId, sourceModId, dependencies, recommended, silent) {
        ...
        return bluebird_1.default.resolve(res)
            // After all phases have queued downloads and phase-gated installers, wait for settlement
            .then((deps) => {
            return this.pollPhaseSettlement(api, sourceModId, {
                phase: undefined,
            ...
            .finally(() => {
            // Wait for all deployment promises to complete before cleanup
            const phaseState = this.mInstallPhaseState.get(sourceModId);
            const deploymentPromises = (phaseState === null || phaseState === void 0 ? void 0 : phaseState.deploymentPromises) || [];
            if (deploymentPromises.length > 0) {
                // Wait for all deployments to complete, then cleanup
                return Promise.all(deploymentPromises)
                    .catch(err => {
                    (0, log_1.log)('warn', 'Error waiting for deployments', { sourceModId, error: err.message });
                })
                    .finally(() => {
                    (0, log_1.log)('debug', 'All deployments complete, cleaning up phase state', { sourceModId });
                    this.mInstallPhaseState.delete(sourceModId);
                });
            }
            else {
                // No deployments to wait for, cleanup immediately
                (0, log_1.log)('debug', 'No deployments pending, cleaning up phase state', { sourceModId });
                this.mInstallPhaseState.delete(sourceModId);
            }
        });
    }
