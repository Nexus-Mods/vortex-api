...
class InstallManager {
        ...
        this.mMainInstallsLimit.do(() => {
            return new Promise((resolve, reject) => {
                ...
                const installationPromise = (0, errorHandling_1.withContext)('Installing', baseName, () => ((forceGameId !== undefined)
                    ...
                    installContext = new InstallContext_1.default(gameId, api, unattended);
    ...
    queueInstallation(api, dep, downloadId, gameId, sourceModId, recommended, phase = 0) {
        ...
        const canStartNow = collectionsInstallWhileDownloading
            ? (phaseNum <= ((_a = phaseState.allowedPhase) !== null && _a !== void 0 ? _a : Number.NEGATIVE_INFINITY))
            : ((phaseState.allowedPhase !== undefined)
                && (phaseNum <= phaseState.allowedPhase)
                && phaseState.downloadsFinished.has(phaseNum));
        if (canStartNow && (download === null || download === void 0 ? void 0 : download.state) === 'finished' && (download === null || download === void 0 ? void 0 : download.size) > 0) {
            startTask();
        }
        else {
            const pending = (_b = phaseState.pendingByPhase.get(phaseNum)) !== null && _b !== void 0 ? _b : [];
    ...
    startQueuedInstallation(api, dep, downloadId, gameId, sourceModId, recommended, phase) {
        ...
        this.mDependencyInstallsLimit.do(() => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const modId = yield this.withInstructions(api, (0, modName_1.default)(sourceMod), (0, modName_1.renderModReference)(currentDep.reference), (_d = (_c = currentDep.reference) === null || _c === void 0 ? void 0 : _c.tag) !== null && _d !== void 0 ? _d : downloadId, (_e = currentDep.extra) === null || _e === void 0 ? void 0 : _e['instructions'], recommended, () => this.installModAsync(currentDep.reference, api, downloadId, { choices: currentDep.installerChoices, patches: currentDep.patches }, currentDep.fileList, gameId, true));
                if (modId) {
                    this.mActiveInstalls.delete(installKey);
                ...
                const endTime = Date.now();
                this.mActiveInstalls.delete(installKey);
                (0, log_1.log)('warn', 'Failed to install dependency', {
                    downloadId,
                    error: err.message,
    ...
    scheduleDeployOnPhaseSettled(api, sourceModId, phase) {
        ...
        const poll = () => {
            ...
            const active = (_a = curr.activeByPhase.get(phase)) !== null && _a !== void 0 ? _a : 0;
            const pending = ((_b = curr.pendingByPhase.get(phase)) !== null && _b !== void 0 ? _b : []).length;
            if ((active === 0) && (pending === 0)) {
                (0, util_1.toPromise)(cb => api.events.emit('deploy-mods', cb))
                    .then(() => {
                    const st = this.mInstallPhaseState.get(sourceModId);
                    if (st) {
                        st.deployedPhases.add(phase);
                        this.maybeAdvancePhase(sourceModId);
                    }
                })
                    .catch(err => (0, log_1.log)('warn', 'deploy-mods failed after phase settle', { sourceModId, phase, error: err === null || err === void 0 ? void 0 : err.message }));
            }
            else {
    ...
    markPhaseDownloadsFinished(sourceModId, phase) {
        this.ensurePhaseState(sourceModId);
        const state = this.mInstallPhaseState.get(sourceModId);
    ...
    maybeAdvancePhase(sourceModId) {
        ...
        while (state.downloadsFinished.has(curr)
            ...
            if ((prev !== undefined) && !state.deployedPhases.has(prev)) {
                (0, log_1.log)('debug', 'phase gating: waiting for previous phase deployment', { sourceModId, prevPhase: prev, currPhase: curr });
                break;
            }
    ...
    doInstallDependenciesPhase(api, dependencies, gameId, sourceModId, recommended, doDownload, abort) {
    ...
    doInstallDependencies(api, gameId, sourceModId, dependencies, recommended, silent) {
            ...
            return dlPromise
                ...
                return (dep.mod == null)
                    ? bluebird_1.default.resolve()
                        .then(() => {
                        var _a;
                        // Queue installation without blocking the download pipeline; respect dependency phase
                        const phaseNum = (_a = dep.phase) !== null && _a !== void 0 ? _a : 0;
                        this.queueInstallation(api, dep, downloadId, gameId, sourceModId, recommended, phaseNum);
        ...
        const res = bluebird_1.default.reduce(phaseList, (prev, depList, idx) => {
            ...
            return this.doInstallDependenciesPhase(api, depList, gameId, sourceModId, recommended, doDownload, abort)
                ...
                this.markPhaseDownloadsFinished(sourceModId, phaseNum);
                return updated;
            })
                .then((updated) => {
                var _a, _b;
        ...
        return bluebird_1.default.resolve(res)
            // After all phases have queued downloads and phase-gated installers, wait outside reducer
            .then((deps) => {
            const POLL_MS = 500; // quick poll; we avoid a long timeout here
            const waitForSettled = () => new bluebird_1.default((resolve) => {
                var _a, _b, _c;
                const phaseState = this.mInstallPhaseState.get(sourceModId);
                if (!phaseState) {
                    return resolve();
                }
                const allowed = (_a = phaseState.allowedPhase) !== null && _a !== void 0 ? _a : 0;
                const active = (_b = phaseState.activeByPhase.get(allowed)) !== null && _b !== void 0 ? _b : 0;
                const pending = ((_c = phaseState.pendingByPhase.get(allowed)) !== null && _c !== void 0 ? _c : []).length;
                if ((active === 0) && (pending === 0)) {
                    resolve();
                }
                else {
                    setTimeout(() => resolve(waitForSettled()), POLL_MS);
                }
            });
