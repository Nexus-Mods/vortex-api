...
exports.getCollectionPhaseProgress = exports.getCollectionCurrentPhase = exports.isCollectionPhaseComplete = exports.getCollectionCompletedMods = exports.getCollectionPendingMods = exports.getCollectionModsInProgress = exports.getCollectionStatusBreakdown = exports.isCollectionModPresent = exports.getCollectionInstallProgress = exports.getCollectionTotalPhases = exports.getCollectionModsForPhase = exports.getCollectionModsByPhase = exports.getCollectionOptionalMods = exports.getCollectionRequiredMods = exports.getCollectionModsByStatus = exports.getCollectionModByReference = exports.getCollectionActiveSessionMod = exports.getCollectionActiveSessionMods = exports.getCollectionSessionMods = exports.isCollectionInstalling = exports.hasCollectionActiveSession = exports.getCollectionLastCompletedSession = exports.getCollectionSessionById = exports.getCollectionSessionHistory = exports.getCollectionLastActiveSessionId = exports.getCollectionActiveSession = void 0;
const reselect_1 = require("reselect");
const path = __importStar(require("path"));
const selectors_1 = require("../mod_management/selectors");
const selectors_2 = require("../download_management/selectors");
/**
...
const getCollectionSessionById = (state, sessionId) => {
    const activeSession = (0, exports.getCollectionActiveSession)(state);
    if ((activeSession === null || activeSession === void 0 ? void 0 : activeSession.sessionId) === sessionId) {
...
const getCollectionSessionMods = (state, sessionId) => {
    var _a;
    const session = (0, exports.getCollectionSessionById)(state, sessionId);
    return (_a = session === null || session === void 0 ? void 0 : session.mods) !== null && _a !== void 0 ? _a : {};
};
exports.getCollectionSessionMods = getCollectionSessionMods;
/**
 ...
 * @returns The mod installation info or undefined if not found
 */
const getCollectionModByReference = (state, searchParams) => {
    const mods = (0, exports.getCollectionActiveSessionMods)(state);
    ...
    return Object.values(mods).find((mod) => {
        var _a, _b, _c, _d;
        const isBundled = ((_b = (_a = mod.rule) === null || _a === void 0 ? void 0 : _a.extra) === null || _b === void 0 ? void 0 : _b.localPath) != null;
        if (isBundled) {
            return (path.basename(mod.rule.extra.localPath) ===
                path.basename(searchParams.logicalFileName, path.extname(searchParams.logicalFileName || "")));
        }
        const ref = (_c = mod.rule) === null || _c === void 0 ? void 0 : _c.reference;
        if (!ref)
            return false;
        // Check each available identifier
        if (searchParams.tag && ref.tag === searchParams.tag)
            return true;
        if (searchParams.fileMD5 && ref.fileMD5 === searchParams.fileMD5)
            return true;
        if (searchParams.fileId && ((_d = ref.repo) === null || _d === void 0 ? void 0 : _d.fileId) === searchParams.fileId)
            return true;
        if (searchParams.logicalFileName &&
            ref.logicalFileName === searchParams.logicalFileName)
            return true;
        return false;
    });
};
exports.getCollectionModByReference = getCollectionModByReference;
/**
...
exports.getCollectionInstallProgress = (0, reselect_1.createSelector)([exports.getCollectionActiveSession], (session) => {
    ...
    const installProgress = session.totalRequired > 0
        ...
        : 0;
    const isComplete = session.installedCount + session.failedCount + session.skippedCount >=
...
exports.isCollectionModPresent = (0, reselect_1.createSelector)([
    (state, collectionSlug) => (0, selectors_1.modsForActiveGame)(state),
    (state, collectionSlug) => (0, selectors_2.activeDownloads)(state),
    (state, collectionSlug) => collectionSlug,
], (mods, downloads, collectionSlug) => {
    const hasDownload = Object.values(downloads).some((dl) => {
        var _a;
        return (dl &&
            dl.modInfo &&
            ((_a = dl.modInfo.attributes) === null || _a === void 0 ? void 0 : _a.collectionSlug) === collectionSlug);
    });
    const hasMod = Object.values(mods).some((mod) => {
        var _a;
        return ["downloaded", "installed"].includes(mod.state) &&
            ((_a = mod.attributes) === null || _a === void 0 ? void 0 : _a.collectionSlug) === collectionSlug;
    });
 ...
 */
exports.getCollectionStatusBreakdown = (0, reselect_1.createSelector)([exports.getCollectionSessionMods], (mods) => {
...
const getCollectionCompletedMods = (state) => {
    const mods = (0, exports.getCollectionActiveSessionMods)(state);
    return Object.values(mods).filter((mod) => mod.status === "installed" ||
        mod.status === "failed" ||
        mod.status === "skipped");
};
...
const isCollectionPhaseComplete = (state, phase) => {
    ...
    return requiredPhaseMods.every((mod) => mod.status === "installed" ||
        mod.status === "failed" ||
        mod.status === "skipped");
};
...
exports.getCollectionPhaseProgress = (0, reselect_1.createSelector)([exports.getCollectionActiveSessionMods], (mods) => {
    ...
    return phases.map((phase) => {
        ...
        const pending = required.filter((m) => m.status === "pending" ||
            m.status === "downloading" ||
            m.status === "downloaded" ||
            m.status === "installing").length;
