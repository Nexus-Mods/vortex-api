    ...
    constructor(api, job, progressCB, finishCB, headersCB, userAgent, throttle, getAgent) {
        ...
        this.stalled = () => {
            if (this.mRequest !== undefined) {
                ...
                (0, log_1.log)('info', 'download stalled, restarting worker', { url: this.mUrl, id: this.mJob.workerId, stallResetsRemaining: this.mStallResets });
                --this.mStallResets;
                this.restart();
            }
        };
                ...
                if (this.mNetworkRetries < DownloadWorker.MAX_NETWORK_RETRIES) {
                    ...
                    setTimeout(() => {
                        if (!this.mEnded) {
                            ...
                            (0, log_1.log)('debug', 'resetting to confirmed position before retry', {
                                id: this.mJob.workerId,
...
class DownloadManager {
    ...
    constructor(api, downloadPath, maxWorkers, maxChunks, speedCB, userAgent, protocolHandlers, maxBandwidth) {
        ...
        this.finishChunk = (download, job, paused) => {
            this.stopWorker(job.workerId);
            (0, log_1.log)('debug', 'stopping chunk worker', { paused, id: job.workerId, offset: job.offset, size: job.size, confirmedSize: job.confirmedSize, confirmedReceived: job.confirmedReceived });
            ...
            if (activeChunk === undefined) {
                ...
                download.assembler.close()
                    ...
                    if (currentExt === '') {
                        ...
                        return bluebird_1.default.resolve(this.detectFileExtensionFromMagic(download.tempName))
                            .then((detectedExt) => {
                            if (detectedExt) {
