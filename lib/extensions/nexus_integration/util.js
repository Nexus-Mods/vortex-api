 ...
 */
function searchCollections(nexus, query, options) {
...
function requestLogin(nexus, api, callback) {
    ...
    return oauth
        .sendRequest((err, token) => __awaiter(this, void 0, void 0, function* () {
...
function startDownloadCollection(api, nexus, urlStr, url, handleErrors = true, referenceTag) {
    ...
    return bluebird_1.default.resolve(nexus.getCollectionRevisionGraph(graphQueries_1.FULL_REVISION_INFO, url.collectionSlug, revNumber))
        ...
        err["revisionNumber"] =
            (_a = revisionInfo === null || revisionInfo === void 0 ? void 0 : revisionInfo.revisionNumber) !== null && _a !== void 0 ? _a : url.revisionNumber;
        if (!handleErrors) {
            return bluebird_1.default.reject(err);
        }
...
function getInfoGraphQL(nexus, domain, modId, fileId) {
    ...
    // } as any;
    return new bluebird_1.default((resolve, reject) => {
        const uid = (0, UIDs_1.makeFileUID)({
            fileId: fileId.toString(),
            modId: modId.toString(),
            gameId: domain,
        });
        if (uid === undefined) {
            return reject(new Error(`Unable to create file UID for game "${domain}", mod ${modId}, file ${fileId}`));
        }
        nexus
...
function transformGraphQLModToIModInfo(graphqlFile) {
    ...
    return {
        ...
        uploaded_users_profile_url: ((_a = mod === null || mod === void 0 ? void 0 : mod.user) === null || _a === void 0 ? void 0 : _a.memberId)
...
function transformGraphQLFileToIFileInfo(graphqlFile) {
    ...
    return {
        id: [
            graphqlFile.fileId || parseInt((_a = graphqlFile.uid) === null || _a === void 0 ? void 0 : _a.split(":")[2], 10) || 0,
        ],
...
function startDownloadMod(api, nexus, urlStr, url, redownload, fileName, allowInstall, handleErrors = true, referenceTag) {
    ...
    return getInfoGraphQL(nexus, pageId, url.modId, url.fileId)
        ...
        return new bluebird_1.default((resolve, reject) => {
            ...
            }, fileName !== null && fileName !== void 0 ? fileName : nexusFileInfo.file_name, (err, downloadId) => (0, util_1.truthy)(err) ? reject((0, errorHandling_1.contextify)(err)) : resolve(downloadId), redownload, { allowInstall });
        });
    })
        .tap(() => {
        api.sendNotification({
        ...
        if (!((_a = state.settings.automation) === null || _a === void 0 ? void 0 : _a.install) &&
            (download === null || download === void 0 ? void 0 : download.state) === "finished") {
            api.sendNotification({
        ...
        if (err.code === "UNKNOWN" && err.errno === 22) {
            ...
            (0, message_1.showError)(api.store.dispatch, "Download failed", detail, {
                allowReport,
            });
        }
...
function endorseDirectImpl(api, nexus, gameId, nexusId, version, endorsedStatus) {
    return (0, endorseMod_1.endorseMod)(nexus, gameId, nexusId, version, endorsedStatus).catch((err) => {
...
function endorseCollectionImpl(api, nexus, gameMode, mod, endorsedStatus) {
    ...
    const nexusCollectionId = mod.attributes.collectionId;
...
function refreshEndorsements(store, nexus) {
    return bluebird_1.default.resolve(nexus.getEndorsements()).then((endorsements) => {
...
function filterByUpdateList(store, nexus, gameId, input) {
    ...
    //  after the update.
    (0, checkModsVersion_1.fetchRecentUpdates)(store, nexus, iterGameId, minAge[iterGameId] || 0).then((entries) => {
        ...
        Object.keys(updateLists).forEach((iterGameId) => {
            updateMap[iterGameId] = updateLists[iterGameId].reduce((prev, entry) => {
                prev[entry.mod_id] =
                    Math.max(entry.latest_file_update, entry.latest_mod_activity) * 1000;
                return prev;
            }, {});
        });
        const now = Date.now();
...
function checkForCollectionUpdates(store, nexus, gameId, mods) {
    const collectionIds = Object.keys(mods).filter((modId) => { var _a; return ((_a = mods[modId].attributes) === null || _a === void 0 ? void 0 : _a.collectionId) !== undefined; });
    return bluebird_1.default.all(collectionIds.map((modId) => {
        ...
        return nexus
            .getCollectionGraph(query, (_a = mod.attributes) === null || _a === void 0 ? void 0 : _a.collectionSlug)
            .then((collection) => {
            var _a;
            ...
            const batched = [
                (0, actions_1.setModAttribute)(gameId, modId, "lastUpdateTime", Date.now()),
            ];
...
function checkForModUpdates(store, nexus, gameId, modsList, forceFull, now) {
    return filterByUpdateList(store, nexus, gameId, modsList).then((filteredMods) => checkForModUpdatesImpl(store, nexus, gameId, modsList, filteredMods, forceFull, now));
}
function checkForModUpdatesImpl(store, nexus, gameId, modsList, filteredMods, forceFull, now) {
    ...
    progress();
    if (forceFull) {
        (0, log_1.log)("info", "[update check] forcing full update check (nexus)", {
            count: modsList.length,
        });
    }
    else {
        (0, log_1.log)("info", "[update check] optimized update check (nexus)", {
    ...
    return bluebird_1.default.map(modsList, (mod) => {
        ...
        return (0, checkModsVersion_1.checkModVersion)(store, nexus, gameId, mod)
            ...
            const newestVerChanged = (0, storeHelper_1.getSafe)(modNew, newWerP, undefined) !==
                (0, storeHelper_1.getSafe)(mod, newWerP, undefined);
            const verChanged = (0, storeHelper_1.getSafe)(modNew, newWerP, undefined) !==
                (0, storeHelper_1.getSafe)(modNew, verP, undefined);
            const newestFileIdChanged = (0, storeHelper_1.getSafe)(modNew, newFileIdP, undefined) !==
                (0, storeHelper_1.getSafe)(mod, newFileIdP, undefined);
            const fileIdChanged = (0, storeHelper_1.getSafe)(modNew, newFileIdP, undefined) !==
                (0, storeHelper_1.getSafe)(modNew, fileIdP, undefined);
...
function checkModVersionsImpl(store, nexus, gameId, mods, forceFull) {
    ...
    const modsList = Object.keys(mods)
        ...
        .filter((mod) => now - ((0, storeHelper_1.getSafe)(mod.attributes, ["lastUpdateTime"], 0) || 0) >
        UPDATE_CHECK_DELAY);
    (0, log_1.log)("info", "[update check] checking mods for update (nexus)", {
        count: modsList.length,
    });
    const updatedIds = [];
...
function getAccountStatus(apiUserInfo) {
    ...
    else if (apiUserInfo.membership_roles.includes("supporter") &&
...
function transformUserInfoFromApi(input) {
    const stateUserInfo = Object.assign({ email: input.email, isPremium: input.membership_roles.includes("premium"), isSupporter: input.membership_roles.includes("supporter"), name: input.name, profileUrl: input.avatar, userId: Number.parseInt(input.sub), isLifetime: input.membership_roles.includes("lifetimepremium"), isBanned: input.group_id === 5, isClosed: input.group_id === 41, status: getAccountStatus(input) }, input.preferences);
...
function updateToken(api, nexus, credentials) {
    ...
    }, (credentials) => onJWTTokenRefresh(api, credentials, nexus)))
