...
function fetchRecentUpdates(store, nexus, gameId, minAge) {
    ...
    if (timeSinceUpdate < UPDATE_CHECK_TIMEOUT &&
        ...
        const period = now - minAge < exports.ONE_DAY ? "1d" : now - minAge < exports.ONE_WEEK ? "1w" : "1m";
        const range = {
            "1d": exports.ONE_DAY,
            "1w": exports.ONE_WEEK,
        ...
        return bluebird_1.default.resolve(nexus.getRecentlyUpdatedMods(period, (0, convertGameId_1.nexusGameId)((0, selectors_1.gameById)(state, gameId), gameId))).then((recentUpdates) => {
...
function checkModVersion(store, nexus, gameMode, mod) {
    ...
    return bluebird_1.default.resolve(nexus.getModFiles(nexusModId, (0, convertGameId_1.nexusGameId)(game, fallBackGameId)))
        ...
        .tapCatch((err) => {
        (0, log_1.log)("warn", "dropping update info", {
            gameMode,
            id: mod.id,
            err: err.message,
        });
...
function updateLatestFileAttributes(dispatch, gameId, mod, file) {
    ...
    if (["OLD_VERSION", "ARCHIVED"].includes(file.category_name) ||
        !(0, util_1.truthy)(file.category_name)) {
    ...
    if (fileUpdates.length === 0) {
        ...
        if (notOld.length === 1 && notOld[0].file_id !== fileId) {
            ...
            if (isPrimary) {
                ...
                const potentialCandidates = files.file_updates.filter((up) => !isFileDeleted(up.new_file_id) &&
                    up.old_file_id === (primaryFile === null || primaryFile === void 0 ? void 0 : primaryFile.file_id));
...
function errorFromNexus(err) {
    if (err.statusCode >= 500) {
        return new Error(`Internal server error (${err.statusCode}, ${err.request}):` +
            err.message);
    }
