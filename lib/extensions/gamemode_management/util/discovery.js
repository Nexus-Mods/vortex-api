...
function queryByArgs(discoveredGames, game) {
    return GameStoreHelper_1.default.find(game.queryArgs)
        .then((results) => bluebird_1.default.all(results.map((res) => fs
...
function handleDiscoveredGame(game, resolvedPath, store, discoveredGames, onDiscoveredGame, onDiscoveredTool) {
    ...
    return (0, getNormalizeFunc_1.default)(resolvedPath)
        ...
        if (err.message !== undefined) {
            (0, log_1.log)("debug", "game not found", {
                id: game.id,
...
function quickDiscovery(knownGames, discoveredGames, onDiscoveredGame, onDiscoveredTool) {
    return bluebird_1.default.all(knownGames.map((game) => quickDiscoveryTools(game.id, game.supportedTools, onDiscoveredTool).then(() => {
        if ((0, storeHelper_1.getSafe)(discoveredGames, [game.id, "pathSetManually"], false)) {
            ...
            return updateManuallyConfigured(discoveredGames, game, onDiscoveredGame).then(() => bluebird_1.default.resolve(undefined));
        }
        ...
        return prom.catch((err) => {
            if (!(err instanceof IGameStore_1.GameEntryNotFound) &&
                ...
                (0, log_1.log)("error", "failed to use game support plugin", {
                    id: game.id,
                    err: err.message,
                    stack: err.stack,
                });
...
function walk(searchPath, matchList, resultCB, progress, normalize) {
        ...
        entries.forEach((entry) => {
            if (entry.isTerminator) {
                if (seenTL.has(entry.filePath)) {
                    ...
                    (0, log_1.log)("debug", "updated estimate", {
                        searchPath,
                        estimatedDirectories,
                        seenDirectories,
    ...
    }, { terminators: true }).then(() => seenDirectories);
}
function verifyToolDir(tool, testPath) {
    ...
    (fileName) => fsExtra.stat(path.join(testPath, fileName)).catch((err) => {
        return bluebird_1.default.reject(err);
    })).then(() => undefined);
}
function assertToolDir(tool, testPath) {
    ...
    return verifyToolDir(tool, testPath)
        ...
        .catch((err) => {
        if (err.code === "ENOENT") {
            (0, log_1.log)("warn", "game directory not valid", {
                game: tool.name,
                testPath,
                missing: err.path,
            });
            ...
            (0, log_1.log)("warn", "game directory can't be read due to file permissions", {
                game: tool.name,
                testPath,
            });
            return testPath;
            ...
            (0, log_1.log)("error", "failed to verify game directory", {
                testPath,
                error: err.message,
            });
        }
...
function discoverRelativeTools(game, gamePath, discoveredGames, onDiscoveredTool, normalize) {
    ...
    return walk(gamePath, matchList, onFileCB, undefined, normalize).then(() => {
        (0, log_1.log)("debug", "done discovering relative tools", {
...
function autoGenIcon(application, exePath, gameId) {
    ...
    return application.logo === "auto"
        ? fs
            .ensureDirWritableAsync(path.dirname(iconPath), () => bluebird_1.default.resolve())
...
function searchDiscovery(knownGames, discoveredGames, searchPaths, onDiscoveredGame, onDiscoveredTool, onError, progressCB) {
    ...
    searchPaths.map((searchPath) => searchPath.endsWith(":") ? searchPath + path.sep : searchPath), (searchPath, index) => {
        ...
        // normalization
        return (0, getNormalizeFunc_1.default)(searchPath, {
            separators: true,
            unicode: false,
            relative: false,
        })
            ...
            .catch((err) => {
            (0, log_1.log)("error", "game search failed", {
                error: err.message,
                searchPath,
            });
