 ...
 */
function quickDiscovery(knownGames, onDiscoveredGame, onDiscoveredTool) {
        ...
        try {
            ...
            prom
                ...
                getNormalizeFunc_1.default(resolvedPath)
                    .then(normalize => discoverRelativeTools(game, resolvedPath, onDiscoveredTool, normalize))
...
const nop = () => undefined;
function discoverRelativeTools(game, gamePath, onDiscoveredTool, normalize) {
...
function testApplicationDirValid(application, testPath, gameId, onDiscoveredGame, onDiscoveredTool, normalize) {
...
function onFile(filePath, files, normalize, onDiscoveredGame, onDiscoveredTool) {
    ...
    for (const match of matches) {
        ...
        testApplicationDirValid(match.application, testPath, match.gameId, onDiscoveredGame, onDiscoveredTool, normalize);
    }
    return false;
}
/**
...
function searchDiscovery(knownGames, discoveredGames, searchPaths, onDiscoveredGame, onDiscoveredTool, progressCB) {
    ...
    searchPaths.map(searchPath => searchPath.endsWith(':') ? searchPath + path.sep : searchPath), (searchPath, index) => {
        ...
        return getNormalizeFunc_1.default(searchPath, { separators: false, unicode: false, relative: false })
            ...
            const matchList = new Set(files.map(entry => path.basename(entry.fileName)));
            const onFileCB = (filePath) => onFile(filePath, files, normalize, onDiscoveredGame, onDiscoveredTool);
