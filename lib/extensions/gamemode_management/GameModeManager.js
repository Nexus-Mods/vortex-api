 ...
 */
class GameModeManager {
    constructor(extensionGames, onGameModeActivated) {
        this.storeGame = (game) => {
            return {
                name: game.name,
        ...
        this.onError = (title, message) => {
            this.mStore.dispatch(notifications_1.addNotification({
                type: 'error',
                message,
                title,
            }));
    ...
    setGameMode(oldMode, newMode) {
        ...
        return fs.statAsync(gameDiscovery.path)
            ...
            .then(normalize => discovery_2.discoverRelativeTools(game, gameDiscovery.path, discoveredGames, this.onDiscoveredTool, normalize))
            .then(() => {
            ...
            return (err.code === 'ENOENT')
                ? Promise.reject(new CustomErrors_1.ProcessCanceled('Missing: ' + (err.filename || modPath)))
                : Promise.reject(err);
        });
    }
    /**
     ...
     */
    startQuickDiscovery() {
        return discovery_2.quickDiscovery(this.mKnownGames, this.mStore.getState().settings.gameMode.discovered, this.onDiscoveredGame, this.onDiscoveredTool);
    }
    /**
    ...
    startSearchDiscovery() {
        ...
        this.mActiveSearch = discovery_2.searchDiscovery(this.mKnownGames, this.mStore.getState().settings.gameMode.discovered, searchPaths, this.onDiscoveredGame, this.onDiscoveredTool, this.onError, progressCallback)
            .finally(() => {
