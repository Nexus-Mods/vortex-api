...
class GameThumbnail extends ComponentEx_1.PureComponentEx {
    ...
    renderMenu() {
        ...
        return (React.createElement("div", { className: 'hover-content' },
            React.createElement(IconBar_1.default, { id: `game-thumbnail-${game.id}`, className: 'buttons', group: `game-${type}-buttons`, instanceId: game.id, staticElements: [], collapse: false, orientation: 'vertical' }),
            React.createElement(OverlayTrigger_1.default, { overlay: gameInfoPopover, triggerRef: this.setRef, getBounds: getBounds || this.getWindowBounds, container: container, orientation: 'horizontal', shouldUpdatePosition: true, trigger: 'click', rootClose: true },
                React.createElement(TooltipControls_1.IconButton, { id: `btn-info-${game.id}`, icon: 'details', className: 'game-thumbnail-info btn-embed', tooltip: t('Show Details') }))));
    }
}
