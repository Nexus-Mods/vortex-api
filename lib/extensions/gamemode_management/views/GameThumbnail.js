...
class GameThumbnail extends ComponentEx_1.PureComponentEx {
    ...
    renderMenu() {
        ...
        const gameInfoPopover = (React.createElement(react_bootstrap_1.Popover, { id: `popover-info-${game.id}`, className: 'popover-game-info' },
            React.createElement(IconBar_1.default, { id: `game-thumbnail-${game.id}`, className: 'buttons', group: `game-${type}-buttons`, instanceId: game.id, staticElements: [], collapse: false, buttonType: 'text', orientation: 'vertical', filter: this.lowPriorityButtons }),
        ...
        return [(React.createElement("div", { key: 'primary-buttons', className: 'hover-content' },
                React.createElement(IconBar_1.default, { id: `game-thumbnail-${game.id}`, className: 'buttons', group: `game-${type}-buttons`, instanceId: game.id, staticElements: [], collapse: false, buttonType: 'text', orientation: 'vertical', filter: this.priorityButtons, clickAnywhere: true }))), (React.createElement(OverlayTrigger_1.default, { key: 'info-overlay', overlay: gameInfoPopover, triggerRef: this.setRef, getBounds: getBounds || this.getWindowBounds, container: container, orientation: 'horizontal', shouldUpdatePosition: true, trigger: 'click', rootClose: true },
