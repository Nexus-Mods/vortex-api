    ...
    return Promise.map(Object.keys(discovered).filter(gameId => discovered[gameId].path !== undefined), gameId => {
        ...
        return stored === undefined
            ...
            : Promise.map(stored.requiredFiles, file => fs.statAsync(path.join(discovered[gameId].path, file)))
                ...
                if (gameMode === gameId) {
                    api.store.dispatch(settings_1.setNextProfile(undefined));
                }
            });
    }).then(() => undefined);
}
...
function genModTypeAttribute(api) {
    return {
        id: 'modType',
        name: 'Mod Type',
        ...
        edit: {
            choices: () => {
                const gameMode = selectors_1.activeGameId(api.store.getState());
                return modTypeExtensions
                    .filter((type) => type.isSupported(gameMode))
                    .map((type) => ({ key: type.typeId, text: (type.typeId || 'Default') }));
            },
            onChangeValue: (mods, newValue) => {
                const gameMode = selectors_1.activeGameId(api.store.getState());
                const setModId = (mod) => {
                    api.store.dispatch(mods_1.setModType(gameMode, mod.id, newValue || ''));
                };
...
function init(context) {
    ...
    context.registerAction('game-icons', 100, 'refresh', {}, 'Quickscan', () => {
        if ($.gameModeManager !== undefined) {
            $.gameModeManager.startQuickDiscovery()
                ...
                const newGames = gameIds.filter(id => (discoveredGames[id] === undefined) || (discoveredGames[id].path === undefined));
    ...
    context.once(() => {
        ...
        $.gameModeManager = new GameModeManagerImpl(extensionGames, (gameMode) => {
        ...
        events.on('start-quick-discovery', (cb) => $.gameModeManager.startQuickDiscovery()
            .then((gameIds) => {
            removeDisapearedGames(context.api);
            if (cb !== undefined) {
                cb(gameIds);
