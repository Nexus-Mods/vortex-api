    ...
    list((error, disks) => {
        if (error) {
            api.showErrorNotification('Failed to determine list of disk drives. ' +
                'Please review the settings before scanning for games.', error);
...
function init(context) {
    ...
    context.registerMainPage('game', 'Games', LazyComponent_1.default('./views/GamePicker', __dirname), {
        hotkey: 'G',
        group: 'global',
        props: () => ({
    ...
    context.registerSettings('Games', LazyComponent_1.default('./views/Settings', __dirname), () => ({
    ...
    context.registerGameInfoProvider('main', 0, 86400000, ['path', 'size', 'size_nolinks'], queryGameInfo_1.default);
    ...
    context.registerAction('game-undiscovered-buttons', 115, 'browse', {}, context.api.translate('Manually Set Location'), (instanceIds) => { browseGameLocation(context.api, instanceIds[0]); });
        ...
        const changeGameMode = (oldGameId, newGameId, oldProfileId) => {
            ...
            return $.gameModeManager.setupGameMode(newGameId)
                ...
                if (err instanceof CustomErrors_1.ProcessCanceled) {
                    message_1.showError(store.dispatch, 'Failed to set game mode', err.message, false, undefined, false);
                }
                else {
                    message_1.showError(store.dispatch, 'Failed to set game mode', err);
                }
                // unset profile
                return store.dispatch(settings_1.setNextProfile(undefined));
            });
        };
